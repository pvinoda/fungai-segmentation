...
def process_masks_for_ui(fmasks):

    unique_labels = np.unique(fmasks)
    label_to_color = {
        label: np.random.randint(0, 256, 3, dtype=np.uint8) for label in unique_labels
    }
    
    # Ensure the background is transparent
    label_to_color[0] = np.array([0, 0, 0], dtype=np.uint8)  # Set RGB to 0 for background

    new_labels_colors= {
        int(label): [int(color[0]), int(color[1]), int(color[2])] for label, color in label_to_color.items()
    }

    rgba_image = np.zeros((*fmasks.shape, 4), dtype=np.uint8)
    for label, color in label_to_color.items():
        rgba_image[fmasks == label, :3] = color
        rgba_image[fmasks == label, 3] = 255  # Set full opacity for all cells
    
    # Set the transparency for the background
    rgba_image[fmasks == 0, 3] = 0
    
    mask_image = Image.fromarray(rgba_image, 'RGBA')
    
    # Save the RGBA image to a PNG format in memory
    buffered = io.BytesIO()
    mask_image.save(buffered, format="PNG")
    mask_png_base64 = base64.b64encode(buffered.getvalue()).decode('utf-8')

    return rgba_image, new_labels_colors, mask_png_base64
...
def get_image_from_sqlite(username: str) -> np.array:
    image = Images.objects.filter(username=username).first()
    if image:
        numpy_array = image.get_image_object()
        print(f"Image Array for {username} has been retrieved from the database.")
        return numpy_array

    else:
        print(f"There are no images corresponding to the user {username}")
        return []


def dump_image_into_sqlite(username: str, image_type: str, image_np_array: np.array, model_type: str) -> None:
    try:
        image: Images = Images(image_type=image_type, model_type=model_type, user=username)
        image.set_image_object(image_np_array)
        image.save()
        print(f"Image has been dumped into database for user {username}")
...
def save_segmentation_results(username, timestamp, action, img, cmasks, cflows):
    overlay, outlines, flowi = show_segmentation(None, img, cmasks, cflows[0], channels=[0, 0])

    directory = "/rs1/researchers/o/oargell/fungai-rs/fungai_out"

    cell_prob = cflows[2]
    _, _, _, processed_cell_prob = process_tiff_image(cell_prob)
    cellprobability = cv2.cvtColor(processed_cell_prob.astype(np.uint8), cv2.COLOR_GRAY2BGR)

    # Save overlay array as pickle
    overlay_filename = os.path.join(directory, f"overlay_{username}_{action}_{timestamp}.pickle")
    with open(overlay_filename, 'wb') as f:
        pickle.dump(overlay, f)

    flow_filename = os.path.join(directory, f"flow_{username}_{action}_{timestamp}.pickle")
    with open(flow_filename, 'wb') as f:
        pickle.dump(flowi, f)

    outline_filename = os.path.join(directory, f"outlines_{username}_{action}_{timestamp}.pickle")
    with open(outline_filename, 'wb') as f:
        pickle.dump(outlines, f)

    cellprob_filename = os.path.join(directory, f"cellprob_{username}_{action}_{timestamp}.pickle")
    with open(cellprob_filename, 'wb') as f:
        pickle.dump(cellprobability, f)

    mask_filename = os.path.join(directory, f"mask_{username}_{action}_{timestamp}.pickle")
    with open(mask_filename, 'wb') as f:
        pickle.dump(cmasks, f)
        
    print(f"Flows, Overlays, Outlines, CellProbs and Masks are saved to the output directory for {action} and {username}")
